...
import { Alert, ActivityIndicator, ScrollView, Switch, View, Text, Modal, TouchableOpacity } from 'react-native';
import { db } from '../../../../../services/firebaseConfig';
import { 
    collection, getDocs, query, where, orderBy, 
    doc, runTransaction, setDoc, Timestamp 
...
export default function RentalForm({ navigation }) {
    ...
    const fetchMotos = async () => {
        try {
            ...
            Alert.alert("Erro", "Falha ao carregar motos disponíveis");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert("Erro", "Falha ao cadastrar aluguel: " + error.message);
        } finally {
            setSubmitting(false);
        }
    };
