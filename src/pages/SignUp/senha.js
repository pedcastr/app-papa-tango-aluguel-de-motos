...
import { Keyboard, Platform, Alert, ActivityIndicator, Pressable, View } from "react-native";
...
export default function Senha({ navigation }) {
    ...
    async function handleConcluir() {
        ...
        if (passwordError || senhaRepetidaErro) {
            showMessage('Atenção', 'Verifique os campos com erros');
            return;
        }
        if (password !== senhaRepetida) {
            setSenhaRepetidaErro("As senhas não coincidem");
        ...
        try {
            ...
            setSucesso(true);
            setTimeout(() => {
                navigation.navigate('Fim do Cadastro', { nome, email });
            }, 1500);
